...
async def generate_stream_link_handler(c: Client, callback_query: CallbackQuery):
    # Get the original message that was clicked on
    original_message = callback_query.message

    # Forward the original message to the LOG_CHANNEL
    log_msg = await original_message.forward(chat_id=LOG_CHANNEL)

    # Rest of your code for generating links
    ...
    await log_msg.reply_text(text=f"**RᴇQᴜᴇꜱᴛᴇᴅ ʙʏ :** [{callback_query.from_user.first_name}](tg://user?id={callback_query.from_user.id})\n**Uꜱᴇʀ ɪᴅ :** `{callback_query.from_user.id}`\n**Stream ʟɪɴᴋ :** {stream_link}", disable_web_page_preview=True,  quote=True)

    # Send the message to the user with generated links
